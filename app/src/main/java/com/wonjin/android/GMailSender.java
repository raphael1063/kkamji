...
public class GMailSender extends javax.mail.Authenticator {
    ...
    public synchronized void sendEmail(String body, String recipients) throws Exception {
        ...
        message.setSubject("[깜지] 인증코드입니다."); //해당 이메일의 본문 설정
    ...
    public synchronized void sendEmailBugReport(String body, String recipients) throws Exception {
        MimeMessage message = new MimeMessage(session);
        DataHandler handler = new DataHandler(new ByteArrayDataSource(body.getBytes(), "text/plain")); //본문 내용을 byte단위로 쪼개어 전달
        message.setSender(new InternetAddress(user));  //본인 이메일 설정
        message.setSubject("[깜지] 버그제보"); //해당 이메일의 본문 설정
        message.setDataHandler(handler);
        if (recipients.indexOf(',') > 0)
            message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(recipients));
        else
